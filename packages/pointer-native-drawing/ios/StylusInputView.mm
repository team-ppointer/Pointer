...
// ---------------------------------------------------------------------------
// StylusInputView — Fabric component for Apple Pencil touch capture.
// Native path building (CatmullRomPathBuilder, NativeDrawingSession) is
// added separately in MAT-362.
// ---------------------------------------------------------------------------
...
- (void)_emitPhase:(int)phase withData:(PencilTouchData *)data
{
  ...
  std::vector<double> xs(count), ys(count), timestamps(count),
                       forces(count), altitudes(count), azimuths(count);
  ...
  std::vector<double> pxs(pCount), pys(pCount), pts(pCount),
                       pforces(pCount), palts(pCount), pazs(pCount);
