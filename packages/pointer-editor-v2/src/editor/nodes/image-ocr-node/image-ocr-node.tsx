...
import { convertMathpixToDollar, recognizeImageWithMathpix } from '../../../api/imageOCR';
...
function useFileUpload(options: UploadOptions) {
  ...
  const uploadFiles = async (files: File[]): Promise<string[]> => {
    ...
    if (options.limit === 1 && fileItems.length >= 1) {
      options.onError?.(new Error('Only one file can be uploaded at a time'));
      return [];
    }

    const filesToUpload = options.limit === 1 ? files.slice(0, 1) : files;
...
interface ImageOCRDragAreaProps {
  onFile: (files: File[]) => void;
  children?: React.ReactNode;
}

const ImageOCRDragArea: React.FC<ImageOCRDragAreaProps> = ({ onFile, children }) => {
...
const DropZoneContent: React.FC = () => (
  <>
    ...
    <div className='tiptap-image-upload-content'>
      <span className='tiptap-image-upload-text'>클릭 또는 드래그 앤 드롭으로 이미지 OCR</span>
    </div>
  </>
);

export const ImageOCRNode: React.FC<NodeViewProps> = (props) => {
  ...
  const runOCR = React.useCallback(
    async (imageUrl: string) => {
      try {
        const result = await recognizeImageWithMathpix(
          imageUrl,
          uploadOptions.ocrApiCall || (async () => ({}))
        );
        ...
        if (isValidPosition(pos)) {
          ...
          uploadOptions.onSuccess?.(baseText);
          focusNextNode(props.editor);
        }
      } catch (err) {
        uploadOptions.onError?.(err instanceof Error ? err : new Error('OCR failed'));
      }
    },
    [props, uploadOptions]
  );
  ...
  const handleUpload = async (files: File[]) => {
    const firstOnly = files && files.length > 0 ? files.slice(0, 1) : [];
    const urls = await uploadFiles(firstOnly);
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    handleUpload(Array.from(files).slice(0, 1));
  };
  ...
  return (
    <NodeViewWrapper className='tiptap-image-upload' tabIndex={0} onClick={handleClick}>
      {!hasFiles && (
        <ImageOCRDragArea onFile={handleUpload}>
          <DropZoneContent />
        </ImageOCRDragArea>
      )}
              ...
              <div className='tiptap-image-upload-preview-content'>
                <div className='tiptap-image-upload-file-info'>
                  ...
                  <div className='tiptap-image-upload-details'>
                    ...
                    <span className='tiptap-image-upload-subtext'>OCR 진행중...</span>
                  </div>
                </div>
