...
// --- Icons ---
import { ListIcon, ListOrderedIcon, ListTodoIcon } from '../../assets';

// --- Lib ---
import { findNodePosition, isNodeInSchema, isNodeTypeSelected, isValidPosition } from '../../utils';

export type ListType = 'bulletList' | 'orderedList' | 'hangulList';
...
export function canToggleList(
  ...
  if (!turnInto) {
    switch (type) {
      ...
      // case 'hangulList':
      //   return editor.can().toggleHangulList();
      default:
        return false;
    }
  }
...
export function isListActive(editor: Editor | null, type: ListType): boolean {
  ...
  switch (type) {
    ...
    case 'hangulList':
      return editor.isActive('hangulList');
    default:
      return false;
  }
}
  ...
  try {
    ...
    if (editor.isActive(type)) {
      ...
      const toggleMap: Record<ListType, () => typeof chain> = {
        ...
        // hangulList: () => chain.toggleHangulList(),
        hangulList: () => chain,
      };
