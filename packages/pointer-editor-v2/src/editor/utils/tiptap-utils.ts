...
import { getFileUploadUrl, uploadFileToS3 } from '../../api/imageUpload';
...
export const handleImageUpload = async (
  ...
  if (isImageUploadInProgress) {
    throw new Error('Another image upload is already in progress');
  }
  ...
  try {
    ...
    const init = await getFileUploadUrl({ fileName: file.name, signal });

    await uploadFileToS3({
      uploadUrl: init.uploadUrl,
      contentDisposition: init.contentDisposition,
      file,
      signal,
      onUploadProgress: (e) => {
    ...
    return init.file.url as string;
  } catch (error: unknown) {
    if (signal.aborted) {
